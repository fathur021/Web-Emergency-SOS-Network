...
async function registerService(input: IRegisterInput): Promise<IAuthResponse> {
  ...
  if (existingUser) {
    throw new AppError(409, "Email atau password sudah terdaftar"); // 409 = Conflict
  }
...
async function loginService(input: ILoginInput): Promise<IAuthResponse> {
  ...
  if (!user) {
    throw new AppError(401, "Email atau password sudah terdaftar"); // 401 = Unauthorized
  }

  // 2. Bandingkan password input dengan hash di database (dilakukan bcrypt, bukan manual)
  const passwordMatch = await bcrypt.compare(input.password, user.password);
  if (!passwordMatch) {
    throw new AppError(401, "Email atau password sudah terdaftar");
  }
